	...
	function SmartBulbNode(config) {
		...
		function setHSB(hsbObject, node) {
			...
			.then(info => {
				...
				hue = hsbObject.hue;
				saturation = hsbObject.saturation;
				brightness = hsbObject.brightness;
				if (hue < 0 || hue > 360) throw ("Hue outsdie of range 0-360");
		...
		node.on('input', function(msg) {
            ...
            if (isPlainObject(msg.payload)) {
				...
				if(msg.topic.includes('state')) {
					...
					.catch(error => {return node.handleConnectionError(error)});
				} else if (msg.topic.includes('brightness')) {
	        		const brightness = parseInt(msg.payload);
	        		setBrightness(brightness, node)
	        			.then(() => node.sendDeviceSysInfo())
	        			.catch(error => {return node.sendError(error)});
				} else if (msg.topic.includes('temperature')){
	        		const temperature = parseInt(msg.payload);
	        		setTemperature(temperature, node)
	        			.then(() => node.sendDeviceSysInfo())
	        			.catch(error => {return node.sendError(error)});
				} else if (msg.topic.includes('hsb')){
					// Expects msg.payload to be a properly formatted HSB object.
	        		setHSB(msg.payload, node)
	        			.then(() => node.sendDeviceSysInfo())
	        			.catch(error => {return node.sendError(error)});
				} else if (msg.topic === 'getInfo') node.sendDeviceSysInfo();
				else if (msg.topic === 'getCloudInfo') node.sendDeviceCloudInfo();
				else if (msg.topic === 'getQuickInfo') node.sendDeviceQuickInfo();
				else if (msg.topic === 'switch') node.deviceInstance.togglePowerState();
				else if (msg.topic === 'getMeterInfo') node.sendDeviceMeterInfo();
				else if (msg.topic === 'clearEvents') context.set('action', msg.payload);
				else if (msg.topic === 'eraseStats') node.sendEraseStatsResult();
				else {
					const actions = msg.topic.split('|');
